...
@property(nonatomic)float  currentJiaodu;
...
- (void)setButtons{
    ...
    for (NSString *key in _imagesAndTitle) {
        ...
        // 则默认image在左，title在右
        // 改成image在上，title在下
        button.titleEdgeInsets = UIEdgeInsetsMake(self.frameWidth/2+4 , -image.size.width-6, 0.0, 0.0);
        button.imageEdgeInsets = UIEdgeInsetsMake(2.0, 8.0, 16.0, -
                                                  button.titleLabel.bounds.size.width + 8);
        button.titleLabel.font = [UIFont systemFontOfSize: self.frameWidth/4];
...
//            NSLog(@"输出----");
    ...
    jiaodu1 = jiaodu;
    if (_currentJiaodu != jiaodu) {
        [self orientChange:nil];
        self.layer.transform = CATransform3DMakeRotation(M_PI*jiaodu1, 0, 0, 1);
        _currentJiaodu = jiaodu;
    }
    
    // CGAffineTransformMakeRotation(M_PI*jiaodu);
}

#pragma mark ------- contentview 操作 --------------------
// 按钮在屏幕右边时，左移contentview, 同时把主按钮往右移
...
- (void)click:(UITapGestureRecognizer*)p
{
    ...
    float controlX = self.frame.origin.x;
    
    switch ( (int)(_currentJiaodu * 10)  ) {
        case 5:
            controlX = self.frame.origin.y;
            width = kScreenHeight;
            break;
        case 10:
            controlX = kScreenWidth - self.frame.origin.x;
            break;
        case 15:
            controlX = kScreenHeight - self.frame.origin.y;
            width = kScreenHeight;
        default:
            break;
    }
    ...
    if (!self.isShowTab) {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            if (controlX <= width/2) {
                ...
                if (_currentJiaodu == 0.5 || _currentJiaodu == 1.5) {
                    // 横着, 这种情况宽高对调
                    float y = self.frame.origin.y;
                    if (_currentJiaodu==1.5) {
                        // home在左边，这种情况是y要往回拉的
                        y = self.frame.origin.y - _imagesAndTitle.count * (self.frameWidth + marginWith);
                    }
                    self.frame = CGRectMake(self.frame.origin.x, y, self.frameWidth, WIDTH + _imagesAndTitle.count * (self.frameWidth + marginWith));
                    
                } else {
                    float x = self.frame.origin.x;
                    if (_currentJiaodu==1) {
                        // home在上边，这种情况是x要往回拉的
                        x = self.frame.origin.x - _imagesAndTitle.count * (self.frameWidth + marginWith);
                    }
                    self.frame = CGRectMake(x, self.frame.origin.y, WIDTH + _imagesAndTitle.count * (self.frameWidth + marginWith) ,self.frameWidth);
                }
                
            }else{
                [self moveContentviewLeft];
                if (_currentJiaodu == 0.5 || _currentJiaodu == 1.5) {
                    // 横着, 这种情况宽高对调, home键在右边，则y左边往左挪
                    float y = self.frame.origin.y  - _imagesAndTitle.count * (self.frameWidth + marginWith);
                    if (_currentJiaodu == 1.5 ) {
                        // home键在右边，y值偏小，需要加大
                    ...
                    self.mainImageButton.frame = CGRectMake((_imagesAndTitle.count * (self.frameWidth + marginWith)), 0, self.frameWidth, self.frameWidth);
                    float x = self.frame.origin.x  - _imagesAndTitle.count * (self.frameWidth + marginWith);
                    if (_currentJiaodu == 1) {
                        // home在上边，x值偏小，需要加大
...
-(void)CloseButtons{
    _contentView.alpha  = 0;
    // 判断主按钮在左边还是右边
    if (self.mainImageButton.frame.origin.x == 0) {
        ...
        } else if( _currentJiaodu == 1.5) {
            self.frame = CGRectMake(self.frame.origin.x , self.frame.origin.y +  _imagesAndTitle.count * (self.frameWidth + marginWith), self.frameWidth ,self.frameWidth);
        } else {
